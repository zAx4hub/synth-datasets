"""synth-datasets engine — zAx4hub (algo=markov)."""
...
def rank(text: str, seed: int = 12) -> float:
    toks = _tokens(text)
    if not toks:
        return 0.0
...
def run(payload: dict[str, Any] | None = None) -> dict[str, Any]:
    payload = payload or {}
    items = payload.get("items") or [{"text": "synth-datasets"}]
    threshold = float(payload.get("threshold", 0.35))
    seed = int(payload.get("seed", 12))
...
def demo() -> dict[str, Any]:
    return run(
        {
            "items": [
                {"text": "Privacy-safe synthetic dataset generator + fidelity reports"},
...
def inspect() -> dict[str, Any]:
    return {
        "name": "synth-datasets",
        "author": "zAx4hub",
        "oneLiner": "Privacy-safe synthetic dataset generator + fidelity reports",
        "algo": "markov",
        "version": "0.1.0",
    }
